/* v17 — squarer floating lyric cards drifting upward through the page, with softer dark fade-out. */
(() => {
'use strict';
if (window.__captionV17) return;
window.__captionV17 = true;
...
.caption-card{position:absolute;top:58%;width:clamp(214px,22vw,272px);min-height:118px;max-height:32vh;will-change:transform,opacity,filter,top;transform:translate3d(0,-50%,0);contain:layout paint;filter:drop-shadow(0 14px 34px #00000044);transition:top .28s linear}
...
.caption-surface{position:relative;min-height:118px;padding:16px 18px;border:1px solid #ffffff46;border-radius:8px;background:linear-gradient(180deg,#fffffff2,#f5fff8db 56%,#eefaf4c7);color:#11161b;box-shadow:0 18px 44px #0005,0 0 24px #ffffff1a,inset 0 1px 0 #ffffffc2;backdrop-filter:blur(12px) saturate(.9);-webkit-backdrop-filter:blur(12px) saturate(.9);overflow:hidden;isolation:isolate;animation:card-float 8.4s ease-in-out infinite alternate;will-change:transform}
...
function change(i,animate=true){
  ...
  if(current){
    ...
    if(animate&&moving()){
      ...
      const a=old.animate([
        ...
        {opacity:.82,offset:.18,transform:`translate3d(${drift*.35}px,calc(-50% - 16px),0) scale(.994)`,filter:'blur(.35px)'},
        {opacity:.42,offset:.48,transform:`translate3d(${drift*.9}px,calc(-50% - 62px),0) scale(.984)`,filter:'blur(2px)'},
        {opacity:0,transform:`translate3d(${drift*1.15}px,calc(-50% - 138px),0) scale(.966)`,filter:'blur(9px)'}
      ],{duration:1440,easing:'cubic-bezier(.2,.75,.22,1)',fill:'forwards'});
  ...
  if(animate&&moving())card.animate([
    {opacity:0,transform:`translate3d(${right?28:-28}px,calc(-50% + 18px),0) scale(.965)`,filter:'blur(9px) saturate(.84)'},
  ...
  ],{duration:1820,easing:'cubic-bezier(.18,.89,.24,1)'});
}
