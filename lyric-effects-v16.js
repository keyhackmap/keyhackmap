/* v16 — cinematic chill finale: layered math, cue bursts, light sweeps, ripple reveal, smooth floating cards. */
(() => {
'use strict';
if (window.__captionV16) return;
window.__captionV16 = true;
...
.caption-card{position:absolute;top:47%;width:clamp(205px,22vw,286px);min-height:108px;max-height:32vh;will-change:transform,opacity,filter;transform:translate3d(0,-50%,0);contain:layout paint;filter:drop-shadow(0 14px 34px #00000044)}
.caption-card.right{right:7vw}.caption-card.left{left:7vw}

.caption-card:before,.caption-card:after{content:'';position:absolute;inset:6px 8px;border-radius:12px;border:1px solid #ffffff1d;background:#dffff20e;z-index:-2;opacity:.45;transform:translate3d(0,7px,0) scale(.985);filter:blur(.1px)}
.caption-card:after{inset:10px 14px;opacity:.22;transform:translate3d(0,14px,0) scale(.97);z-index:-3}
.caption-ripple{position:absolute;left:50%;top:50%;width:40px;height:40px;border:1px solid #cffff28a;border-radius:50%;transform:translate(-50%,-50%) scale(.35);opacity:0;pointer-events:none;box-shadow:0 0 20px #7cf3d13d}
...
.caption-halo{position:absolute;inset:-18px;border-radius:18px;background:radial-gradient(circle at 50% 50%,#8df7dc4f 0,#8df7dc18 36%,transparent 72%);filter:blur(22px);opacity:.34;animation:halo-breathe 7s ease-in-out infinite alternate;will-change:transform,opacity}
@keyframes halo-breathe{0%{transform:scale(.94) translateY(3px);opacity:.22}100%{transform:scale(1.05) translateY(-3px);opacity:.42}}
.caption-surface{position:relative;min-height:108px;padding:15px 18px;border:1px solid #ffffff46;border-radius:12px;background:linear-gradient(180deg,#fffffff2,#f4fff8d8 58%,#edfff4bc);color:#11161b;box-shadow:0 18px 44px #0005,0 0 24px #ffffff1a,inset 0 1px 0 #ffffffc2;backdrop-filter:blur(12px) saturate(.9);-webkit-backdrop-filter:blur(12px) saturate(.9);overflow:hidden;isolation:isolate;animation:card-float 8.4s ease-in-out infinite alternate;will-change:transform}
  ...
  if(current){
    ...
    if(animate&&moving()){
      ...
      const a=old.animate([
        ...
        {opacity:.88,offset:.22,transform:`translate3d(${drift*.25}px,calc(-50% - 10px),0) scale(.996)`,filter:'blur(.3px)'},
        {opacity:.55,offset:.58,transform:`translate3d(${drift*.7}px,calc(-50% - 40px),0) scale(.988)`,filter:'blur(1.5px)'},
        {opacity:0,transform:`translate3d(${drift}px,calc(-50% - 96px),0) scale(.97)`,filter:'blur(8px)'}
      ],{duration:1820,easing:'cubic-bezier(.18,.72,.22,1)',fill:'forwards'});
